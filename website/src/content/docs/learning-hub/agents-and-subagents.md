...
Yes, as of v1.0.72, **multi-turn subagents are always enabled**. You can send follow-up messages to a subagent that is still running, without waiting for it to finish its current task. This makes interactive refinement of subagent work possible mid-flight — useful for redirecting a subagent that has gone off-track or providing additional context it needs to complete a task.
