...
*(v1.0.72+)* Use `/model --session` (or `/model -s`) to change the model, reasoning effort, or context window for **just the current session**, leaving global settings unchanged. This is ideal for one-off experiments where you want a different model without permanently changing your configuration:

```
/model --session                    # open model picker for the current session only
/model -s claude-opus-4-5           # switch to a specific model for this session
```

*(v1.0.74+)* Use `/model --plan` (or `/model plan`) to pick a model used specifically **while in plan mode**. Pass a model ID to set it, `off` to clear, or no argument to open the picker. The plan-mode model reverts to the session model when you leave plan mode:

```
/model --plan                       # open model picker for plan mode
/model --plan claude-haiku-4-5      # use a fast model for planning
...
The `/worktree` command (v1.0.61+) creates a new git worktree and switches into it, **leaving your uncommitted changes behind** in the current branch. Use this when you want to start parallel work on a new branch with a clean slate:
...
The `/move` command *(v1.0.72+)* is the companion to `/worktree` — it creates a new git worktree and **carries your uncommitted changes into it**. Use `/move` when you have work in progress that you want to continue on a new branch:

```
/move my-feature-branch
```

| Command | What it does with uncommitted changes |
|---------|--------------------------------------|
| `/worktree` | Leaves them behind in the current branch |
| `/move` | Carries them into the new worktree |
...
> **Plan mode improvements (v1.0.74+)**: Plan mode now **hard-blocks built-in tool calls that would modify the workspace** — the agent cannot edit files or run mutating shell commands while planning. Planning artifacts written to the session folder (e.g., `~/.copilot/session-state/<sessionId>/files/`) are still allowed. This makes plan mode a safe, read-only proposal phase where you can review the agent's intent before approving execution.
