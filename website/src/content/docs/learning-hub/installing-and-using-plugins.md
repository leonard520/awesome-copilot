...
| **MCP Servers** | Model Context Protocol integrations for external tools | `.mcp.json`, `.github/mcp.json`, or `mcp.json` (v1.0.74+) |
...
> **Open Plugin Spec v1 (v1.0.74+)**: GitHub Copilot CLI now supports **Open Plugin Spec v1** plugin manifests in addition to the native `plugin.json` format. This means plugins authored for other AI coding tools using the Open Plugin Spec can be installed and used directly in Copilot CLI without conversion. When a plugin uses `mcp.json` (a standard MCP configuration file), Copilot CLI will read it automatically alongside `.mcp.json` and `.github/mcp.json`.
